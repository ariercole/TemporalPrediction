...
manualDensityPlots <- function(mids, cleanData, title = TRUE, minititles = TRUE, plotDir = "densityPLots_manual", width = 4000, height = 2000) {
  for(title in unique(filter(allVariablesDF, outlierRemoval == TRUE, day > 0, !(variableTitle %in% c("NoradrenalineTotal", "AdrenalineTotal", "VasopressinTotal")))$variableTitle)) {
    ...
    for(minMax in c("min", "max")) {
      ...
      for (day in 1:(nrow(varDF)/2)) {
        ...
        text(y = 1.25 * max(d$y), x = max(d$x), pos = 2, labels = paste("Original values: ", sum(!wherePos), sep = ""), col = "darkblue", cex = 0.9)
        text(y = 1.15 * max(d$y), x = max(d$x), pos = 2, labels = paste("Imputed values: ", sum(imputedPoints), sep = ""), col = "#A3C1AD", cex = 0.9)
      }
    }
