...
day.cumul <- data.frame(day = c(1,2,2,3,3,4,4,5,5), cumul = c(F,F,T,F,T,F,T,F,T)) # define the days on which to build models

tuneMachineLearningModels(Iter = iter, DeepIter = deep.iter, MLmethods = ML.methods, seed.list = seed.list, path.D = path.save, dayList = day.cumul)
...
for (MLmethod in c("adaboost", "APACHE", "avNNet", "DeepNN", "glm", "parRF", "svmRadialWeights")) {
  assign(paste("ML.results.df.", MLmethod, sep = ""), buildPredResDF(MLmethod = MLmethod,  pred.res = "results", model.df = model.df.all, path = path.save))
}
...
tune.grid.adaboost <- expand.grid(method = "Adaboost.M1", nIter = 2000)
tune.grid.DeepNN <- expand.grid(complexity.multiplier = c(seq(from = 3/16, to = 20/16, by = 1/16)),
                                ...
                                rate.dropout_6 = 0.2,
                                Epochs = 30)
tune.grid.svmRadialWeights <- expand.grid(C = c(0.1,0.3,0.5,0.8), Weight = c(2,3,4,5,7.5,10), sigma = c(0.001,0.025,0.005,0.01))
...
for (MLmethod in c("adaboost", "APACHE", "avNNet", "DeepNN", "glm", "parRF", "svmRadialWeights")) {
  assign(paste("ML.resampleResults.df.", MLmethod, sep = ""), getBestTunePreds(MLmethod, path = path.save) %>% generateCustomSummary())
}
...
plotMLOutputGraph(plotMetric = "AUROC", plotName = "AUROC", resampleResults = ML.resampleResults.df.ALL, day.range = 5, MLmethods = c("glm", "parRF", "adaboost", "avNNet", "svmRadialWeights", "DeepNN"), file.name = "AUROC_allMLmethods", lines.plot = FALSE)
